...
public class OrderServiceImpl implements OrderService {
    ...
    public Long createOrder(OrderInfo orderInfo) {
        ...
        for (CartProduct cartProduct : list) {
            ...
            //删除购物车中含该订单的商品
            try {
                List<Object> jsonList=  redisTemplate.opsForHash().values(CART_PRE + ":" + orderInfo.getUserId());
//
                for (Object json : jsonList) {
                    CartProduct cart = new Gson().fromJson(json.toString(), CartProduct.class);
                    if (cart.getProductId().equals(cartProduct.getProductId())) {
                        redisTemplate.opsForHash().delete(CART_PRE + ":" + orderInfo.getUserId(), cart.getProductId() + "");
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
